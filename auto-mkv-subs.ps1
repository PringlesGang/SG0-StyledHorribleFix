...
function Find-MkvMerge {
    while ($null -eq (Get-Command -Name $global:MkvMerge -ErrorAction SilentlyContinue)) {
        Write-Host -ForegroundColor Red "Could not execute mkvmerge with command ""$global:MkvMerge"""
